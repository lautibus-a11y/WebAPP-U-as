...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F9] flex flex-col lg:flex-row">
      ...
      {/* Mobile Top Header */}
      <header className="lg:hidden bg-white border-b border-[#FBCACA]/10 px-6 py-4 flex justify-between items-center sticky top-0 z-30">
        <div>
          <div className="text-xl font-serif text-[#C5A059]">Bellezza</div>
          <div className="text-[8px] uppercase tracking-widest text-gray-400 font-bold">Admin</div>
        </div>
        <button onClick={onLogout} className="p-2 text-red-400 hover:bg-red-50 rounded-xl transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
        </button>
      </header>

      {/* Mobile Bottom Navigation */}
      <nav className="lg:hidden fixed bottom-6 left-6 right-6 bg-white/80 backdrop-blur-xl border border-white/40 shadow-2xl rounded-3xl flex justify-around p-2 z-40">
        {[
          { id: 'bookings', label: 'Citas', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2V7a2 2 0 002-2h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V7z' },
          { id: 'servicios', label: 'Servicios', icon: 'M4 6h16M4 10h16M4 14h16M4 18h16' },
          { id: 'gallery', label: 'Fotos', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' }
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)}
            className={`flex flex-col items-center gap-1.5 px-6 py-3 rounded-2xl transition-all ${activeTab === tab.id ? 'bg-[#C5A059] text-white shadow-lg shadow-[#C5A059]/20' : 'text-gray-400'}`}
          >
      ...
      {/* Main Content Area */}
      <main className="flex-1 lg:ml-72 p-6 md:p-8 lg:p-16 pb-32 lg:pb-16">
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 md:mb-12 gap-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-serif text-[#1A1A1A]">{activeTab === 'bookings' ? 'Gestión de Citas' : activeTab === 'servicios' ? 'Servicios' : 'Galería'}</h1>
              <p className="text-[10px] uppercase tracking-widest text-gray-400 mt-2 font-bold">Panel de Control</p>
            </div>
            {activeTab !== 'bookings' && (
              <button
                onClick={() => { setEditingItem(null); setPreviewUrl(null); setIsModalOpen(true); }}
                className="w-full md:w-auto bg-[#C5A059] text-white px-8 py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl shadow-[#C5A059]/20 hover:scale-[1.02] active:scale-95 transition-all text-center"
              >
                Añadir Nuevo
              </button>
            )}
            ...
            <div className="animate-in fade-in duration-500">
              {activeTab === 'bookings' && (
                <div className="grid gap-4">
                  {bookings.map(b => (
                    <div key={String(b.id)} className="bg-white p-5 md:p-6 rounded-[32px] border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm hover:shadow-md transition-shadow group gap-6">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 md:gap-3">
                          <div className="font-bold text-gray-900 text-base md:text-lg">{b.customer_name}</div>
                        ...
                        <div className="text-[11px] md:text-xs text-[#C5A059] font-medium mt-2 uppercase tracking-wider flex items-center gap-2">
                          <span className="truncate max-w-[150px]">{b.service_name}</span>
                          <span className="text-gray-300">•</span>
                          <span className="whitespace-nowrap">{b.appointment_date}</span>
                          <span className="text-gray-300">|</span>
                          <span className="whitespace-nowrap">{b.appointment_time}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        <select
                          ...
                          className={`flex-1 sm:flex-none text-[10px] font-bold uppercase p-3 px-4 rounded-xl border outline-none cursor-pointer transition-colors ${b.status === 'confirmed' ? 'bg-green-50 text-green-600 border-green-100' : b.status === 'cancelled' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-orange-50 text-orange-600 border-orange-100'}`}
                        >
